...
- 로그인 사용자와 비로그인 사용자를 모두 고려하되, 과제에서는 사용자 식별 전략의 큰 흐름까지만 다룬다.
- 품절/구매 제한 상품은 없다고 가정한다.
...
- 추천 모델 학습 결과 또는 상품 점수는 별도 배치/ML 시스템에서 생성되어 저장되어 있다고 가정한다.
- 추천시스템은 이벤트 수집, 추천 후보 조회, 랭킹, 응답 제공, 성과 측정을 책임진다.
- 추천 지면은 메인 페이지에서 호출되며, 한 번의 요청에 최대 10개 상품을 반환한다.
- 이벤트 수집과 추천 응답은 서로 직접 의존하지 않도록 비동기 처리 구조를 고려한다.
- 품절, 판매 중지, 노출 불가 상품은 추천 결과에서 제외되어야 한다.
...
- 추천 결과는 사용자별 관심도, 상품 인기도, 최신 이벤트, 비즈니스 정책을 함께 고려해 정렬되어야 한다.
- 추천 후보가 부족한 경우 인기 상품, 카테고리 인기 상품 등 fallback 결과를 제공할 수 있어야 한다.
- 품절, 판매 중지, 성인 상품 등 노출 불가 상품을 필터링할 수 있어야 한다.
- 추천 결과 노출, 클릭, 장바구니 담기 이벤트를 연결해 CTA 성과를 측정할 수 있어야 한다.
- 이벤트 처리 실패 또는 중복 수집 상황에서도 추천 결과와 성과 지표가 크게 왜곡되지 않아야 한다.
- 캐시를 활용해 반복 요청에 대한 응답 속도를 개선할 수 있어야 한다.
