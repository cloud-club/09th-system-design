...
반도체 Fab의 증착 장비, 식각 장비, 검사 장비 등에서 온도, 압력, 가스 유량, RF Power, 장비 운영 로그, Alarm 이벤트가 지속적으로 발생한다.

이번 시스템은 장비를 직접 제어하거나 수율 예측 AI 모델을 학습하는 시스템이 아니다. 목표는 설비에서 발생하는 대량 이벤트를 안정적으로 수집하고, 실시간 스트림 처리로 이상 징후를 탐지하며, Dashboard와 알림으로 엔지니어가 빠르게 확인할 수 있게 하는 **FDC(Fault Detection and Classification) 성격의 모니터링 시스템**이다.

#### FabGuard 참고 사례

FabGuard는 글로벌 정밀 측정 기기 기업 INFICON에서 개발한 반도체 및 디스플레이 제조 산업용 FDC(Fault Detection and Classification) 소프트웨어 솔루션이다. 공정 장비의 방대한 센서 데이터를 실시간으로 수집하고 분석해 불량과 장비 이상을 사전에 감지하는 것을 목표로 한다.

주요 기능은 압력, 유량, 온도 등 장비 내 센서 데이터를 기반으로 한 실시간 공정 모니터링, 정상 공정 데이터(Golden Data)와 현재 공정 데이터의 비교를 통한 결함 탐지와 조기 경보, 결함 원인 분류 및 분석, 설비 성능 지표 추적과 클린 레시피 실행 간격 모니터링이다.

이번 과제에서는 실제 INFICON FabGuard 제품을 그대로 구현하는 것이 아니라, 그 FDC 문제의식을 참고해 **제조 설비 이벤트 수집, 이상 탐지, 장애 복구와 재처리 구조**에 집중한 백엔드 인프라 아키텍처로 단순화해 설계한다.
